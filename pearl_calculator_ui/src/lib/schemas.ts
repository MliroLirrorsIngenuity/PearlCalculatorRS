...
export const PearlTraceResultSchema = z.object({
    ...
    pearl_trace: z.array(z.object({ X: z.number(), Y: z.number(), Z: z.number() })),
...
export const WizardTNTConfigSchema = z.object({
    northWest: z.object({ x: z.string().min(1), y: z.string().min(1), z: z.string().min(1) }),
    northEast: z.object({ x: z.string().min(1), y: z.string().min(1), z: z.string().min(1) }),
    southWest: z.object({ x: z.string().min(1), y: z.string().min(1), z: z.string().min(1) }),
    southEast: z.object({ x: z.string().min(1), y: z.string().min(1), z: z.string().min(1) }),
    redTNTLocation: z.string().min(1),
});

export const WizardBitConfigSchema = z.object({
    state: BitInputStateSchema.nullable().optional(),
    skipped: z.boolean(),
}).refine((data) => {
    if (data.skipped) return true;
    if (!data.state) return false;
    const hasEmptyValue = data.state.sideValues.some((v) => !v || v.trim() === "");
