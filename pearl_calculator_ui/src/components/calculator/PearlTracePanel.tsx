			...
			<div className="w-[40%] h-full">
				<Card className="h-full">
					<CardHeader className="pb-3">
						<CardTitle className="text-base">{t("calculator.trace_summary_title")}</CardTitle>
					</CardHeader>
					<CardContent className="space-y-2 text-sm">
						<div className="flex flex-col gap-1">
							<span className="text-muted-foreground">{t("calculator.trace_dest_target")}</span>
						...
						<div className="flex flex-col gap-1">
							<span className="text-muted-foreground">{t("calculator.trace_tnt_config")}</span>
							<div className="grid grid-cols-3 gap-2 font-medium">
								<span>{t("calculator.trace_blue", { value: traceTNT?.blue ?? "N/A" })}</span>
								<span>{t("calculator.trace_red", { value: traceTNT?.red ?? "N/A" })}</span>
								<span>{t("calculator.trace_total", { value: traceTNT?.total ?? "N/A" })}</span>
							</div>
						</div>
						{closestApproach && (
							<>
								<div className="flex flex-col gap-1">
									...
									<div className="grid grid-cols-3 gap-2 font-medium">
										<span>{t("calculator.trace_x", { value: closestApproach.point.X.toFixed(2) })}</span>
										<span>{t("calculator.trace_y", { value: closestApproach.point.Y.toFixed(2) })}</span>
										<span>{t("calculator.trace_z", { value: closestApproach.point.Z.toFixed(2) })}</span>
									</div>
								</div>
								<div className="flex flex-col gap-1">
									<span className="text-muted-foreground">
								...
								<div className="flex flex-col gap-1">
									...
									<span className="font-medium">
										{closestApproach.distance.toFixed(2)} {t("calculator.suffix_blocks")}
									</span>
								</div>
							</>
						)}
