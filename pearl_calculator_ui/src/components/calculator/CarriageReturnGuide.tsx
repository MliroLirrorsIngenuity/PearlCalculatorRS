...
export function CarriageReturnGuide({
    ...
    return (
        <div
            className={`h-4 w-full border-dotted mb-1 flex ${isRightToLeft
        ...
        >
            <div
                className={`${isRightToLeft ? "mt-[-7px] mr-2" : "mb-[-7px] ml-2"}`}
            >
                {isRightToLeft ? (
                    <ArrowLeft
                        className={`w-3 h-3 ${theme === "blue" ? "text-blue-300" : "text-red-300"
                            }`}
                    />
                ) : (
                    <ArrowRight
